...
def main() -> int:
    ...
    parser.add_argument("--cohort-name", default="recent Hugging Face generators")
    ...
    if sample_count == 0 or len(manifest) != sample_count:
        raise RuntimeError("diagnostic manifest must contain unique samples")
    ...
    expected_prediction_count = sample_count * len(VIEW_ORDER)
    if len(prediction_rows) != expected_prediction_count or actual_keys != expected_keys:
        raise RuntimeError(f"prediction ledger does not have exact {sample_count} x 3 coverage")
    ...
    samples_per_model = collections.Counter(
        row["model_id"] for row in enriched if row["view"] == "clean"
    )
    by_model = {}
    ...
    report = {
        ...
        "cohort": {
            ...
            "real_samples": 0,
            "generators": len(model_ids),
            "samples_per_generator": dict(sorted(samples_per_model.items())),
        ...
        "integrity": {
            ...
            "prediction_coverage": f"{expected_prediction_count}/{expected_prediction_count}",
        ...
        "cross_view": {
            ...
            "all_three_views_recall": all_views_detected / sample_count,
            "at_least_one_view_detected": at_least_one_detected,
            "at_least_one_view_recall": at_least_one_detected / sample_count,
        },
    ...
    lines = [
        f"# Frozen Sieve FT1 on {args.cohort_name}",
        "",
        ...
        f"({by_view['clean']['detected']}/{sample_count:,}; Wilson 95% CI "
    ...
    for view in VIEW_ORDER:
        item = by_view[view]
        lines.append(
            f"| {view} | {item['detected']}/{sample_count:,} | {pct(item['recall'])} | "
    ...
    for model_id in model_ids:
        ...
        model_count = samples_per_model[model_id]
        lines.append(
            f"| `{model_id}` | {items['clean']['detected']}/{model_count} ({pct(items['clean']['recall'])}) | "
            f"{items['web']['detected']}/{model_count} ({pct(items['web']['recall'])}) | "
            f"{items['hard']['detected']}/{model_count} ({pct(items['hard']['recall'])}) | "
    ...
    lines += [
        ...
        "",
        f"- Detected in all three views: {all_views_detected}/{sample_count:,} ({pct(all_views_detected / sample_count)}).",
        f"- Detected in at least one view: {at_least_one_detected}/{sample_count:,} ({pct(at_least_one_detected / sample_count)}).",
        f"- Clean misses: {len(clean_misses)}; see `clean_misses.csv`.",
        "",
        "## Protocol and interpretation",
        "",
        f"The frozen FP16 ONNX graph (`{report['integrity']['onnx_sha256']}`) and untouched Platt calibrator were used once, with no tuning on this cohort. The combined train/calibration/diagnostic manifest passed exact and pHash-distance-4 cross-role leakage checks. All {expected_prediction_count:,} predictions completed without failure.",
        "",
        f"This is a positive-only diagnostic cohort: it measures recall on {len(model_ids)} generators. Balanced accuracy, specificity, false-positive rate, and AUROC are undefined because there are no real images. It therefore cannot establish overall detector quality or competition performance.",
    ]
